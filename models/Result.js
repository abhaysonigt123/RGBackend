...
resultSchema.methods.recalculate = function () {
  ...
  // Determine if Primary Class (1, 2, KG) - REMOVED (Standardized)
  
  // Helper to calculate subject total
  const getSubjectTotal = (sub) => {
    if (!sub) return 0;
    return (sub.c1Written || 0) + (sub.c1Oral || 0) + (sub.c2Written || 0) + (sub.c2Oral || 0);
  };

  // Get all unique subjects
  const allSubjects = new Set();
  if (this.term1) this.term1.forEach(s => allSubjects.add(s.subject));
  if (this.term2) this.term2.forEach(s => allSubjects.add(s.subject));

  let totalOfAverages = 0;

  allSubjects.forEach(subjectName => {
    const t1Sub = this.term1 ? this.term1.find(s => s.subject === subjectName) : null;
    const t2Sub = this.term2 ? this.term2.find(s => s.subject === subjectName) : null;

    const t1Marks = getSubjectTotal(t1Sub);
    const t2Marks = getSubjectTotal(t2Sub);

    // Average of Term 1 and Term 2
    const avg = (t1Marks + t2Marks) / 2;
    totalOfAverages += avg;
  });

  // Grand Total is now the Sum of Averages
  this.grandTotal = Number(totalOfAverages.toFixed(2));
  
  // Calculate percentage
  // Max marks per subject is 100 (as we took average of terms)
  // Percentage = (Grand Total / (Count * 100)) * 100
  const subjectsCount = allSubjects.size;
